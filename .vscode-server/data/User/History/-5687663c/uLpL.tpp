...
void BalancedList<T>::add(const T& item) {
    if (head == nullptr) {
        head = new Node(initialArraySize);
        tail = head;
    }

    tail->array.add(item);

    if (tail->array.size() > tail->array.getCapacity()) {
        size_t half = tail->array.size() / 2;

        Node* newNode = new Node(initialArraySize);
        for (size_t i = half; i < tail->array.size(); ++i) {
            newNode->array.add(tail->array.get(i));
        }

        for (size_t i = tail->array.size(); i > half; --i) {
            tail->array.remove(i - 1);
        }
...
T BalancedList<T>::remove() {
    ...
    T removedValue = head->array.remove(0);
    totalSize--;
...
T BalancedList<T>::remove(size_t index) {
    rangeCheck(index);
    Node* current = head;
    while (index >= current->array.size()) {
