...
TEST(StudentTest, Validation) {
    TestStudent s;
    s.setScores({90, 85, 100}, {80, 70, 60}, 95);
    EXPECT_NO_THROW(s.validate());
    
    s.setScores({110}, {80, 70, 60}, 95);
    EXPECT_THROW(s.validate(), std::domain_error);
}
TEST(StudentTest, Validation) {
    TestStudent s1;
    s1.setScores({90, 85, 100}, {80, 70, 60}, 95);
    EXPECT_NO_THROW(s1.validate()); // Should not throw

    TestStudent s2;
    s2.setScores({110}, {80, 70, 60}, 95); // Quiz out of range
    EXPECT_THROW(s2.validate(), std::domain_error);

    TestStudent s3;
    s3.setScores({90, 85, 100}, {80, 70, 60}, -5); // Final score out of range
