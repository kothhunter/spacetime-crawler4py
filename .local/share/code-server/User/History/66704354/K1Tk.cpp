...
TEST_F(Student, ValidateScores) {
    ...
    student.quiz = {110}; // Invalid score
    EXPECT_THROW(student.validate(), std::domain_error);

    student.hw = {-5}; // Invalid score
    EXPECT_THROW(student.validate(), std::domain_error);

    student.final_score = 102; // Invalid final score
    EXPECT_THROW(student.validate(), std::domain_error);
}

// Test quiz average calculation (dropping lowest)
TEST_F(StudentTest, ComputeQuizAverage) {
    student.quiz = {80, 90, 70, 85};
    student.compute_quiz_avg();
    EXPECT_DOUBLE_EQ(student.quiz_avg, (80 + 90 + 85) / 3.0); // Drops 70
}

// Test homework average calculation
...
// Test input operator for Student
...
// Test output operator for Student
...
// Test computing grades in Gradebook
...
// Test input operator for Gradebook
...
// Test output operator for Gradebook
