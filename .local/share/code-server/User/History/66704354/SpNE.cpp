...
// // TEST(StudentTest, InvalidQuizScore) {
// //     Student s;
// //     s.quiz = {90, 110, 70}; // 110 is out of range
// //     s.hw = {85, 95, 100};
// //     s.final_score = 88;
// //     EXPECT_THROW(s.validate(), std::domain_error);
// // }

// // // Test invalid homework score
// // TEST(StudentTest, InvalidHwScore) {
// //     Student s;
// //     s.quiz = {90, 80, 70};
// //     s.hw = {85, -5, 100}; // -5 is out of range
// //     s.final_score = 88;
// //     EXPECT_THROW(s.validate(), std::domain_error);
// // }

// // // Test invalid final score
// // TEST(StudentTest, InvalidFinalScore) {
// //     Student s;
// //     s.quiz = {90, 80, 70};
// //     s.hw = {85, 95, 100};
// //     s.final_score = 105; // 105 is out of range
// //     EXPECT_THROW(s.validate(), std::domain_error);
// // }

// // // Test multiple invalid scores
// // TEST(StudentTest, MultipleInvalidScores) {
// //     Student s;
// //     s.quiz = {90, 101, 70}; // 101 is out of range
// //     s.hw = {85, 95, -10}; // -10 is out of range
// //     s.final_score = 200; // 200 is out of range
// //     EXPECT_THROW(s.validate(), std::domain_error);
// // }

// TEST(StudentInputTest, ReadFromStringStream) {
//     std::stringstream str{"Name Student\nQuiz 1 2 3\nHW 1 2\nFinal 3\n"};
//     Student student;
//     str >> student;
//     EXPECT_EQ(student.first_name, "Student");
//     EXPECT_EQ(student.quiz, std::vector<int>({1, 2, 3}));
//     EXPECT_EQ(student.hw, std::vector<int>({1, 2}));
//     EXPECT_EQ(student.final_score, 3);
// }
...
// Test reading input from a stringstream
TEST(StudentInputTest, ReadFromStringStream) {
    std::stringstream str{"Name John Doe\nQuiz 90 85 95\nHW 80 100\nFinal 92"};
    Student student;
    str >> student;  // This will populate quiz, hw, and final_score

    // Since we can't access private members directly, we assume correctness via behavior
    EXPECT_NO_THROW(student.validate());
}

// Test computation indirectly by checking output
TEST(StudentComputeTest, ComputeCourseScore) {
    std::stringstream str{"Name John Doe\nQuiz 100 90 80\nHW 100 80 90\nFinal 90"};
    Student student;
    str >> student;  // Populate student data
    student.compute_grade();
