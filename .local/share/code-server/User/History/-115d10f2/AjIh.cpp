...
TEST(StringFunction, strchr) {
    EXPECT_EQ(String::strchr("boo", 'f'), "boo");
    EXPECT_EQ(String::strchr("boo", 'o'), "oo"); 
    EXPECT_EQ(String::strchr("boo", 'x'), nullptr); 
    EXPECT_EQ(String::strchr("", 'x'), nullptr);
}

TEST(StringFunction, strstr) {
    const char *haystack = "foobar";
    EXPECT_EQ(String::strstr(haystack, "foo"), haystack); // Substring at start
    EXPECT_EQ(String::strstr(haystack, "bar"), haystack + 3); // Substring at end
    EXPECT_EQ(String::strstr(haystack, "oba"), haystack + 2); // Substring in middle
    EXPECT_EQ(String::strstr(haystack, "xyz"), nullptr); // Substring not found
    EXPECT_EQ(String::strstr(haystack, ""), haystack); // Empty needle
}
