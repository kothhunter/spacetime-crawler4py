...
// Check if exact change is available
...
// Extract exact change if possible
Coins Coins::extract_exact_change(const Coins& coins) {
    if (!has_exact_change_for_coins(coins)) {
        throw invalid_argument("Not enough coins to extract exact change.");
    }
    quarters -= coins.quarters;
    dimes -= coins.dimes;
    nickels -= coins.nickels;
...
// Determine the coins required for a given amount in cents
...
// Print an amount in cents as dollars and cents
void print_cents(int cents, ostream& out) {
    out << "$" << (cents / 100) << "." << (cents % 100);
}

// Prompt the user for coin input
...
// Menu for interacting with the Coins object
