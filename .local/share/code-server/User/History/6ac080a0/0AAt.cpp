...
std::string clean_word(const std::string& word) {
    ...
    // If no alphabetic character is found, return an empty string
    if (start >= end) return "";

    // Extract the word from start to end
    std::string cleaned(start, end);
    
    // Remove any characters that are not alphabetic or hyphens
    cleaned.erase(std::remove_if(cleaned.begin(), cleaned.end(), [](char c) {
        return !std::isalpha(c) && c != '-';
    }), cleaned.end());
...
std::map<std::string, int> count_words(std::istream& document, const std::set<std::string>& stopwords) {
    ...
    while (document >> word) {
        word = clean_word(word); // Clean the word to remove unwanted characters
        to_lowercase(word); // Convert the cleaned word to lowercase
