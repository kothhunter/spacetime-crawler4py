...
TEST(WordCount, CountWords) {
    ...
    EXPECT_EQ(expecting, count_words(second_input, words1));

    istringstream third_input("I HOPE THIS PASSES");
    set<string> words2 = {"i", "hope", "this", "passes"};
    map<string, int> expect2 = {};
    EXPECT_EQ(expect2, count_words(third_input, words2));
}

TEST(WordCount, OutputWordCounts) {
    stringstream output0;
    map<string, int> expecting = {{"is",2},{"a",1},{"test",1}};
    output_word_counts(expecting,output0);
    EXPECT_STREQ(output0.str().c_str(), "a 1\nis 2\ntest 1\n");

    stringstream output1;
    map<string, int> expecting = {};
    output_word_counts(expecting, output1);
