...
TEST(ListTests, Compare) {
    ...
    EXPECT_EQ(list::compare(list1, list2), 0); // Equal strings
    EXPECT_LT(list::compare(list1, list3), 0); // "abc" < "abd"
    EXPECT_GT(list::compare(list3, list1), 0); // "abd" > "abc"
...
// Test finding a substring in a list
...
TEST(ListTests, Nth) {
    ...
    EXPECT_EQ(list::nth(list, 5), nullptr); // Out of bounds
