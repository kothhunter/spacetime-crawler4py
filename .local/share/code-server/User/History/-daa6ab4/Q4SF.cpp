...
TEST(ListTests, FromStringEmpty) {
    Node* list = list::from_string("");
...
TEST(ListTests, CopyEmpty) {
    Node* empty_list = list::from_string("");
    Node* copied_list = list::copy(empty_list);
    
    EXPECT_EQ(copied_list, nullptr); 
    list::free(copied_list);
    list::free(empty_list);
}
...
TEST(ListTests, AppendEmptyToEmpty) {
    Node* list1 = list::from_string("");
    Node* list2 = list::from_string("");
    Node* combined = list::append(list1, list2);
...
TEST(ListTests, AppendEmptyToNonEmpty) {
    Node* list1 = list::from_string("Hello");
    Node* list2 = list::from_string("");
    Node* combined = list::append(list1, list2);

    EXPECT_EQ(list::compare(combined, list1), 0);
    EXPECT_EQ(list::length(combined), 5);

    list::free(list1);
    list::free(combined);
}

TEST(ListTests, AppendNonEmptyToEmpty) {
    Node* list1 = list::from_string("");
    Node* list2 = list::from_string("World");
    Node* combined = list::append(list1, list2);

    EXPECT_EQ(list::compare(combined, list2), 0);
    EXPECT_EQ(list::length(combined), 5);
...
TEST(ListTests, FindCharEmpty) {
    Node* list = list::from_string("");
    EXPECT_EQ(list::find_char(list, 'a'), nullptr);
    list::free(list);
}

TEST(ListTests, FindCharNotPresent) {
    Node* list = list::from_string("abcdef");
    EXPECT_EQ(list::find_char(list, 'z'), nullptr);
    list::free(list);
}
