#include "compute_grades.hpp"

#include <algorithm>
#include <numeric>
#include <iomanip>
#include <stdexcept>
#include <cmath>
#include <ranges>

void Student::validate() const {
    if (!std::all_of(quiz.begin(), quiz.end(), [](int score) { return score >= 0 && score <= 100; }) ||
        !std::all_of(hw.begin(), hw.end(), [](int score) { return score >= 0 && score <= 100; }) ||
        !(final_score >= 0 && final_score <= 100)) {
        throw std::domain_error("Error: invalid score found");
    }
}

void Student::compute_quiz_avg() {
    if (quiz.empty()) quiz_avg = 0;
    else if (quiz.size() == 1) quiz_avg = quiz[0];
    else {
        int min_score = *std::min_element(quiz.begin(), quiz.end());
        int total = std::accumulate(quiz.begin(), quiz.end(), 0) - min_score;
        quiz_avg = static_cast<double>(total) / (quiz.size() - 1);
    }
}

//     quiz_avg = quiz.empty() ? 0 : static_cast<double>(std::accumulate(quiz.begin(), quiz.end(), 0) - (quiz.size() > 1 ? *std::min_element(quiz.begin(), quiz.end()) : 0)) / std::max(1, static_cast<int>(quiz.size()) - 1);
// }


void Student::compute_hw_avg() {
    hw_avg = hw.empty() ? 0 : static_cast<double>(std::accumulate(hw.begin(), hw.end(), 0)) / hw.size();
}

void Student::compute_course_score() {
    compute_quiz_avg();
    compute_hw_avg();
    double total = (0.4 * quiz_avg) + (0.3 * hw_avg) + (0.3 * final_score);
    course_score = static_cast<int>(std::round(total));
}

void Student::compute_grade() {
    // compute_quiz_avg();
    // compute_hw_avg();
    compute_course_score();
    // const std::tuple<int, std::string> grades[] = {
    const std::pair<int, std::string> grades[] = {
        {97, "A+"}, {93, "A"}, {90, "A-"},
        {87, "B+"}, {83, "B"}, {80, "B-"},
        {77, "C+"}, {73, "C"}, {70, "C-"},
        {67, "D+"}, {63, "D"}, {60, "D-"}, {0, "F"}
    };
    
    for (const auto& [limit, grade] : grades) {
        if (course_score >= limit) {
            course_grade = grade;
            break;
        }
    }
}

std::strong_ordering Student::operator<=>(const Student& other) const {
    return std::tie(last_name, first_name) <=> std::tie(other.last_name, other.first_name);
}
//     if (auto cmp = last_name <=> other.last_name; cmp != 0) return cmp;
//     return first_name <=> other.first_name;
// }

bool Student::operator==(const Student& other) const {
    return last_name == other.last_name && first_name == other.first_name;
}

std::istream& operator>>(std::istream& in, Student& s) {
    // std::string line, label;
    s.first_name = "Missing";
    s.last_name = "Name";
    s.quiz.clear();
    s.hw.clear();
    s.final_score = 0;

    std::string line;
    while (std::getline(in, line) && !line.empty()) {
        std::istringstream ss(line);
        std::string key;
        ss >> key;

        if (key == "Name") {
            std::string part;
            if (ss >> s.first_name && ss >> s.last_name) {
                std::string extra;
                while (ss >> extra) {
                    s.last_name += " " + extra;
                }
            }
        } else if (key == "Quiz") {
            s.quiz = std::vector<int>(std::istream_iterator<int>(ss), {});
        } else if (key == "HW") {
            s.hw = std::vector<int>(std::istream_iterator<int>(ss), {});
        } else if (key == "Final") {
            ss >> s.final_score;
        }
    }
    return in;
}
//variation
//     while (std::getline(in, line) && !line.empty()) {
//         std::istringstream ss(line);
//         ss >> label;
//         if (label == "Name") {
//             ss >> s.first_name >> s.last_name;
//         } else {
//             std::vector<int>* target = nullptr;
//             if (label == "Quiz") target = &s.quiz;
//             else if (label == "HW") target = &s.hw;

//             if (target) {
//                 int score;
//                 while (ss >> score) {
//                     target->push_back(score);
//                 }
//             } else if (label == "Final") {
//                 ss >> s.final_score;
//             }
//         }
//     }
//     return in;
// }

std::ostream& operator<<(std::ostream& out, const Student& s) {
    out << std::left << std::setw(8) << "Name:"  << s.first_name + " " + s.last_name << "\n"
        << std::setw(8) << "HW Ave:" << static_cast<int>(std::round(s.hw_avg)) << "\n"
        << std::setw(8) << "QZ Ave:" << static_cast<int>(std::round(s.quiz_avg)) << "\n"
        << std::setw(8) << "Final:"  << static_cast<int>(std::round(s.final_score)) << "\n"
        << std::setw(8) << "Total:"  << s.course_score << "\n"
        << std::setw(8) << "Grade:"  << s.course_grade << "\n\n";
    return out;
}
//     out << "Name:   " << std::left << std::setw(16) << (s.first_name + " " + s.last_name) << '\n'
//         << "HW Ave: " << std::left << std::setw(8) << static_cast<int>(std::round(s.hw_avg)) << '\n'
//         << "QZ Ave: " << std::left << std::setw(8) << static_cast<int>(std::round(s.quiz_avg)) << '\n'
//         << "Final:  " << std::left << std::setw(8) << static_cast<int>(std::round(s.final_score)) << '\n'
//         << "Total:  " << std::left << std::setw(8) << static_cast<int>(s.course_score) << '\n'
//         << "Grade:  " << s.course_grade << '\n';
//     return out;
// }


void Gradebook::compute_grades() {
    std::for_each(students.begin(), students.end(), [](Student& s) { s.compute_grade(); });
}

void Gradebook::sort() {
    std::ranges::sort(students);
}
//     std::sort(students.begin(), students.end());
// }

void Gradebook::validate() const {
    std::for_each(students.begin(), students.end(), [](const Student& s) { s.validate(); });
}

std::istream& operator>>(std::istream& in, Gradebook& b) {
    Student s;
    while (in >> s) {
        b.students.push_back(s);
    }
    return in;
}

std::ostream& operator<<(std::ostream& out, const Gradebook& b) {
    std::for_each(b.students.begin(), b.students.end(), [&out](const Student& s) { out << s << "\n"; });
    return out;
}
