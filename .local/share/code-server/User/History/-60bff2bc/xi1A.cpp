...
void Student::validate() const {
    ...
    auto invalidQuizIter = std::find_if(quizScores.begin(), quizScores.end(), isOutOfRange);
    auto invalidHwIter = std::find_if(homeworkScores.begin(), homeworkScores.end(), isOutOfRange);
    bool isFinalInvalid = isOutOfRange(totalFinalScore);

    if (invalidQuizIter != quizScores.end()) {
        std::stringstream errorMsg;
        errorMsg << "Error: invalid percentage " << *invalidQuizIter;
        throw std::domain_error(errorMsg.str());
    }
    if (invalidHwIter != homeworkScores.end()) {
        std::stringstream errorMsg;
    ...
    if (isFinalInvalid) {
        std::stringstream errorMsg;
        errorMsg << "Error: invalid percentage " << totalFinalScore;
        throw std::domain_error(errorMsg.str());
    }
}
