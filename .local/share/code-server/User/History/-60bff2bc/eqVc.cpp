...
std::ostream& operator<<(std::ostream& out, Gradebook& b) {
    b.compute_grades(); // Ensure all students have computed grades before printing

    std::for_each(b.students.begin(), b.students.end(), [&out](const Student& s) { 
        out << s << "\n"; 
    });
